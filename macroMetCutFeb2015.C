...
std::vector<TH1F *> CreateESMassShape(TTree * fullTreeSgn, TString processName, double scaleFactor){
  ...
  TH1F *h6 = new TH1F ("hTemp5","hTemp distribution",50,70,120);
  TH1F *h7 = new TH1F ("hTemp6","hTemp distribution",50,70,120);
  hTemp.push_back (h1);
  hTemp.push_back (h2);
  hTemp.push_back (h3);
  hTemp.push_back (h4);
  ...
  double weight_MuID=0;
 double weight_MuIso=0;

 /* if((TMath::Abs(tag_eta)<0.8) && (tag_pt<30)) {weight_MuID= MuID_eta1[0];  weight_MuIso= MuIso_eta1[0];}
 if((0.8<TMath::Abs(tag_eta)<1.2) && (tag_pt<30)) {weight_MuID= MuID_eta2[0]; weight_MuIso= MuIso_eta2[0];}
 if((1.2<TMath::Abs(tag_eta)<2.1) && (tag_pt<30)) {weight_MuID= MuID_eta3[0]; weight_MuIso= MuIso_eta3[0];}
  
 if((TMath::Abs(tag_eta)<0.8) && (tag_pt>30))   {weight_MuID= MuID_eta1[1]; weight_MuIso= MuIso_eta1[1];}
 if((0.8<TMath::Abs(tag_eta)<1.2) && (tag_pt>30)) {weight_MuID= MuID_eta2[1]; weight_MuIso= MuIso_eta2[1];}
 if((1.2<TMath::Abs(tag_eta)<2.1) && (tag_pt>30)) {weight_MuID= MuID_eta3[1]; weight_MuIso= MuIso_eta3[1];}
 */

  double weightTotal= weight_MuIso*weight_MuID;

  TLorentzVector Tag; Tag.SetPtEtaPhiM(0.,0.,0.,0.);
  TLorentzVector Probe; Probe.SetPtEtaPhiM(0.,0.,0.,0.);

  TLorentzVector Tag_Up; Tag_Up.SetPtEtaPhiM(0.,0.,0.,0.);
  TLorentzVector Probe_Up; Probe_Up.SetPtEtaPhiM(0.,0.,0.,0.);

  TLorentzVector Tag_Down; Tag_Down.SetPtEtaPhiM(0.,0.,0.,0.);
  TLorentzVector Probe_Down; Probe_Down.SetPtEtaPhiM(0.,0.,0.,0.);

  TLorentzVector Pair_TauUp, Pair_TauDown, Pair_MuUp, Pair_MuDown, Pair;
 
  /* if( processName =="ZL" ){
    TH1F *hResUp = new TH1F ("hResUp","hResUp",50,70,120);
    TH1F *hResDown = new TH1F ("hResDown","hResDown",50,70,120);
  ...
  for (Int_t i=0; i<nentries; i++) {
    ...
    double px = (tag_pt)*TMath::Cos(tag_phi);
    double py = (tag_pt)*TMath::Sin(tag_phi);
    double pz = (tag_pt)*TMath::SinH(tag_eta);

    double Taupx = (pt)*TMath::Cos(phi);
    double Taupy = (pt)*TMath::Sin(phi);
    double Taupz = (pt)*TMath::SinH(eta);

    double muMass  =   TMath::Sqrt( tag_En*tag_En -  (px*px+ py*py + pz*pz)   );
    double tauMass  =  TMath::Sqrt( En*En -  (Taupx*Taupx+ Taupy*Taupy + Taupz*Taupz)   );


    Tag.SetPtEtaPhiM(tag_pt, tag_eta, tag_phi, muMass);
    Probe.SetPtEtaPhiM(pt, eta, phi, tauMass);

    Tag_Up.SetPtEtaPhiM( (tag_pt*(1.01)), (tag_eta*(1.01)), (tag_phi*(1.01)), (muMass*(1.01)) );
    Probe_Up.SetPtEtaPhiM( (pt*(1.03)), (eta*(1.03)), (phi*(1.03)), (tauMass*(1.03)) );

    Tag_Down.SetPtEtaPhiM( (tag_pt*(0.99)), (tag_eta*(0.99)), (tag_phi*(0.99)), (muMass*(0.99)) );
    Probe_Down.SetPtEtaPhiM( (pt*(0.97)), (eta*(0.97)), (phi*(0.97)), (tauMass*(0.97)) );
    ...
    double    METx_shiftedUp = MET.Px() - ( Probe.Px() -  Probe_Up.Px());
    double    METy_shiftedUp = MET.Py() - ( Probe.Py() -  Probe_Up.Py());

    double    METx_shiftedDown = MET.Px() - (Probe.Px() - Probe_Down.Px());
    double    METy_shiftedDown = MET.Py() - (Probe.Py() - Probe_Down.Py());

    double MET_shiftedUp = TMath::Sqrt( METx_shiftedUp*METx_shiftedUp  + METy_shiftedUp*METy_shiftedUp);
    double MET_shiftedDown =  TMath::Sqrt( METx_shiftedDown*METx_shiftedDown + METy_shiftedDown*METy_shiftedDown);
    ...
    if(scaleFactor != 1.){
      if((Probe_Up.Pt()>20.6)   && tag_pt > 25 && Mt_up<40 &&  MET_shiftedUp<25 )     {hTemp[0]->Sumw2();hTemp[0]->Fill(Pair_TauUp.M(), tag_puMCWeightRun2012 );}
      if(Probe_Down.Pt()>20.6 && tag_pt > 25 && Mt_down<40 && MET_shiftedDown<25)       {hTemp[1]->Sumw2();hTemp[1]->Fill(Pair_TauDown.M(), tag_puMCWeightRun2012);}
      if(Probe.Pt()>20.6 && Tag_Up.Pt()>25   && tag_Mt<40  && tag_muMVAMet<25)        {hTemp[2]->Sumw2();hTemp[2]->Fill(Pair_MuUp.M(), tag_puMCWeightRun2012);}
      if(Probe.Pt()>20.6 && Tag_Down.Pt()>25 && tag_Mt<40 && tag_muMVAMet<25)        {hTemp[3]->Sumw2();hTemp[3]->Fill(Pair_MuDown.M(), tag_puMCWeightRun2012);}
      if((Probe.Pt()>20.6) && (tag_pt>25) && (tag_Mt<40) && tag_muMVAMet<25)         {hTemp[4]->Sumw2();hTemp[4]->Fill(Pair.M(), tag_puMCWeightRun2012);}
      if((Probe.Pt()>20.6) && (tag_pt>25) && (tag_Mt<40) && tag_muMVAMet<25)         {hTemp[5]->Sumw2();hTemp[5]->Fill((1.20*Pair.M() - 0.20*mZ), tag_puMCWeightRun2012);}
      if((Probe.Pt()>20.6) && (tag_pt>25) && (tag_Mt<40) && tag_muMVAMet<25)         {hTemp[6]->Sumw2();hTemp[6]->Fill((0.80*Pair.M() + 0.20*mZ), tag_puMCWeightRun2012);}
    }else{
      if(Probe_Up.Pt()>20.6 && tag_pt > 25 && Mt_up<40 && MET_shiftedUp<25 )     {hTemp[0]->Fill(Pair_TauUp.M() );}
      if(Probe_Down.Pt()>20.6 && tag_pt > 25 && Mt_down<40 && MET_shiftedDown<25)  {hTemp[1]->Fill(Pair_TauDown.M());}
      if(Probe.Pt()>20.6 && Tag_Up.Pt()>25 && tag_Mt<40 && tag_muMVAMet<25)        {hTemp[2]->Fill(Pair_MuUp.M());}
      if(Probe.Pt()>20.6 && Tag_Down.Pt()>25 && tag_Mt<40 && tag_muMVAMet<25)      {hTemp[3]->Fill(Pair_MuDown.M());}
      if((Probe.Pt()>20.6) && (tag_pt > 25) && (tag_Mt<40) && tag_muMVAMet<25)     {hTemp[4]->Fill(Pair.M());}
    }
  }
  ...
  for(int i=0; i<50;i++){
    ...
    //cout<<i<<" MassHist: "<<hTemp[4]->GetBinContent(i)<<" Mass down: "<<hTemp[3]->GetBinContent(i)<<" Mass up: "<<hTemp[2]->GetBinContent(i)<<endl;
  ...
  if(scaleFactor != 1.){
    ...
    hTemp[5]->Sumw2();hTemp[5]->Scale(scaleFactor*0.987883333*0.985533333*0.9548436313);
    hTemp[6]->Sumw2();hTemp[6]->Scale(scaleFactor*0.987883333*0.985533333*0.9548436313);
  }  else {
    hTemp[0]->Scale(1.);
    hTemp[1]->Scale(1.);
    hTemp[2]->Scale(1.);
  ...
  for(int i=0; i<50;i++){
    ...
    //cout<<i<<" MassBin: "<<binc[i]<<" Mass down: "<<binc_MuDown[i]<<" Mass up: "<<binc_MuUp[i]<<endl;
  }


  for(int i=0;i<(int)hTemp.size(); i++){
    cout<<" Integral ES "<<hTemp[i]->Integral()<<endl;
  }

  for(int k=0; k<5; k++){
    hTemp[k]->SetBinContent(0, 0.);
    hTemp[k]->SetBinError(0, 0.);
    hTemp[k]->SetBinContent(hTemp[k]->GetNbinsX() + 1, 0.);
    hTemp[k]->SetBinError(hTemp[k]->GetNbinsX() + 1, 0.);
  }


  if( processName =="ZL" ){
    hTemp[0]->Write(processName+"_CMS_scale_faket_mutau_8TeVUp");
    hTemp[1]->Write(processName+"_CMS_scale_faket_mutau_8TeVDown");
    ...
    hTemp[5]->Write(processName+"_ZL_CMS_res_mutau_8TeVUp");
    hTemp[6]->Write(processName+"_ZL_CMS_res_mutau_8TeVDown");
  } else{
...
TH1F * histoProducer(TTree * fullTreeSgnCut, float ScaleFactorSgn, float weight, std::string var, int min, int max, int num){
  ...
  hMass->SetBinContent(0, 0.);
  hMass->SetBinError(0, 0.);
  hMass->SetBinContent(hMass->GetNbinsX() + 1, 0.);
  hMass->SetBinError(hMass->GetNbinsX() + 1, 0.);
...
void fitStudyTemplatesFromMC(
			     ...
			     const std::string additionalCut_ = "(tag_muPFIsolation < 0.1 && abseta < 2.3 && DecayMode > 0.5 && HpsLooseCombIsoDBCorr3Hits > 0.5 && pair_charge == 0  && tag_muTriggerMatching > 0.5 && tag_triggerBitSingleMu > 0.5)",
			     const std::string additionalCutSS_ = "(pt>20.6 && tag_pt > 25 && tag_muPFIsolation < 0.1 && abseta < 2.3 && DecayMode > 0.5 && HpsLooseCombIsoDBCorr3Hits > 0.5 && pair_charge != 0 && tag_Mt<40 && tag_muTriggerMatching > 0.5 && tag_triggerBitSingleMu > 0.5) && tag_muMVAMet<25",
			     const std::string additionalCutHiMt_ = "(pt>20.6 && tag_pt > 25 && tag_muPFIsolation < 0.1 && abseta < 2.3 && DecayMode > 0.5 && HpsLooseCombIsoDBCorr3Hits > 0.5 && pair_charge == 0 && tag_Mt > 60 && tag_muTriggerMatching > 0.5 && tag_triggerBitSingleMu > 0.5)"
			     ){
  ...
  string path = "/lustre/cms/store/user/calabria/Data/MuToTau_25May2014_MC/Sum/";
  string pathData = "/lustre/cms/store/user/calabria/Data/MuToTau_25May2014_Data/Sum/";
  ...
  //  hsgnDataHistTempNo->Write("zmumuJets");
    ...
    std::vector<TH1F*> zmumuShiftAll = CreateESMassShape(reducedTrees["zmm"],"zmumuAll",scaleFactors["zmm"]);
    std::vector<TH1F*> zmumuShiftJets = CreateESMassShape(reducedTrees["zmmJets"],"zmumuJets",scaleFactors["zmm"]);
  ...
  std::vector<TH1F*> zmumuFailShiftall = CreateESMassShape(reducedTreesF["zmm"],"zmumuAll",scaleFactors["zmm"]);
  std::vector<TH1F*> zmumuFailShiftJets = CreateESMassShape(reducedTreesF["zmmJets"],"zmumuJets",scaleFactors["zmm"]);
